...
let total = $derived.by(() => {
    let soma = 0;
    // Usar um ciclo for clássico evita que o compilador reclame do tipo do 'item'
    ...
    // Se tentar entrar aqui sem nada no carrinho, volta para os eventos
    if ($carrinho.length === 0) {
        if (typeof window !== 'undefined') goto('/eventos');
    }
</script>

<main class="checkout-page">
    <form method="POST" action="?/finalizarReserva" use:enhance>
